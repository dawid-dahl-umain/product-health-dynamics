import { defaultScenarios, scenarioKeys, simulateScenario } from './simulation'
...
const formatResults = (stats: ReturnType<typeof simulateScenario>) => [
  ...
  `Average trajectory (first 10): ${stats.averageTrajectory.slice(0, 10).join(', ')}`,
].join('\n')

const main = () => {
  const { scenario, runs } = parseArgs()
  if (!scenarioKeys.includes(scenario as typeof scenarioKeys[number])) {
    console.error(`Unknown scenario "${scenario}". Options: ${scenarioKeys.join(', ')}`)
    process.exit(1)
  }
